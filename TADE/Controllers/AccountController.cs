...
namespace TADE.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    Session["role"] = role;
                    if (role == "WorkProvider" || role == "WorkSeeker" || role == "WorkSeekerAndProvider")
                    {
                        var companyDetails = db.CompanyDetails.Where(a => a.UserId == userdt.userID).Select(x => new
                        {
                            companyId = x.CompanyId
                        }).FirstOrDefault();
                        if (companyDetails != null)
                        {
                            Session["CompanyId"] = companyDetails.companyId;
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> CompanyRegister(CompanyDetailViewodel companyDetailViewModel)
        {
            if (this.IsCaptchaValid("Captcha is not valid"))
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    RegisterViewModel rModel = new RegisterViewModel();
                    rModel.Email = companyDetailViewModel.Email;
                    rModel.Password = companyDetailViewModel.Password;
                    rModel.ConfirmPassword = companyDetailViewModel.Password;
                    rModel.Role = companyDetailViewModel.RoleType;
                    var user = new ApplicationUser { UserName = rModel.Email, Email = rModel.Email };
                    var result = await UserManager.CreateAsync(user, rModel.Password);
                    if (result.Succeeded)
                    {
                        ...
                        //string path = Path.Combine(Server.MapPath("~/Images/CompanyLogos"), Path.GetFileName(file.FileName));
                        var fileName = Path.GetFileName(companyDetailViewModel.File.FileName);
                        var path = Path.Combine(Server.MapPath("~/Images/CompanyLogos"), fileName);
                        companyDetailViewModel.File.SaveAs(path);
                        CompanyDetail companyDetail = new CompanyDetail();
                        companyDetail.UserId = user.Id;
                        companyDetail.CompanyName = companyDetailViewModel.CompanyName;
                        companyDetail.AddressLIne1 = companyDetailViewModel.AddressLIne1;
                        companyDetail.AddressLIne2 = companyDetailViewModel.AddressLIne2;
                        companyDetail.AddressLIne3 = companyDetailViewModel.AddressLIne3;
                        companyDetail.PostCode = companyDetailViewModel.PostCode;
                        companyDetail.Email = companyDetailViewModel.Email;
                        companyDetail.Password = companyDetailViewModel.Password;
                        companyDetail.PhoneNumber = companyDetailViewModel.PhoneNumber;
                        companyDetail.ContactPerson = companyDetailViewModel.ContactPerson;
                        companyDetail.Position = companyDetailViewModel.Position;
                        companyDetail.CompanyLogo = fileName;
                        companyDetail.NatureofBusiness = companyDetailViewModel.NatureofBusiness;
                        companyDetail.CompanySize = companyDetailViewModel.CompanySize;
                        companyDetail.Status = true;
                        companyDetail.RoleType = companyDetailViewModel.RoleType;
                        companyDetail.AboutTheCompany = companyDetailViewModel.AboutTheCompany;
                        companyDetail.DetailedDescriptionAboutCompany = companyDetailViewModel.DetailedDescriptionAboutCompany;
                        
                        db.CompanyDetails.Add(companyDetail);
                        db.SaveChanges();
                        Session["CompanyId"] = Convert.ToInt32(companyDetail.CompanyId);
                        // SendMail(candidateDetail);
                        //
